...
{
    const int total = num_boxes * crop_height * crop_width * depth;
    cudaError_t err;
    int total_blocks = (total+kThreadsPerBlock-1)/kThreadsPerBlock;

    if( total_blocks > 0 )
    {
        CropAndResizeKernel<<<total_blocks,kThreadsPerBlock>>>( total, image_ptr, boxes_ptr, box_ind_ptr, num_boxes,
                                                                batch, image_height, image_width, crop_height, crop_width,
                                                                depth, extrapolation_value, crops_ptr );
                                     ...
                                     float *grads_image_ptr )
{
    ...
    if( total_blocks>0 )
    {
        CropAndResizeBackpropImageKernel<<<total_blocks,kThreadsPerBlock>>>( total, grads_ptr, boxes_ptr, box_ind_ptr, num_boxes,
                                                                             batch, image_height, image_width, crop_height,
