...
class AnchorBoxesCoder():
    ...
    def encode(self, pos_anchors, pos_boxes):
        num_per_im = [len(box) for box in pos_boxes]
        pos_anchors = torch.cat(pos_anchors, dim=0)
        pos_boxes = torch.cat(pos_boxes, dim=0)
        targets = self.encode_once(pos_anchors, pos_boxes)
