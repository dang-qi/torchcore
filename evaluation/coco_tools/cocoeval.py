...
class COCOeval:
    ...
    def accumulate(self, p = None, print_info=True):
        '''
    ...
    def summarize(self, print_info=True):
        '''
    ...
    def get_per_image_result(self, imgIds=None, sort_key=None, descending=False):
        ''' if imgIds is None, do it for all image
            sort key can be 'AP@all', 'AP@0.5', 'AP@0.75', 'AP small', 'AP medium', 'AP large',
                     'AR@1', 'AR@10', 'AR@100', 'AR samll', 'AR medium', 'AR large' for detection task'''
        if imgIds is None:
            imgIds=sorted(self.cocoGt.getImgIds())
        
        results = []
        for imgId in imgIds:
            self.params.imgIds = [imgId]
            self.evaluate(print_info=False)
            self.accumulate(print_info=False)
            self.summarize(print_info=False)
            single_result = self._set_single_result(imgId)
            results.append(single_result)

        if sort_key is not None:
            results=sorted(results, key=lambda i:i[sort_key],reverse=descending)
        return results
    
    def _set_single_result(self, im_id):
        iouType=self.params.iouType
        if iouType == 'segm' or iouType == 'bbox':
            metrics=['AP@all', 'AP@0.5', 'AP@0.75', 'AP small', 'AP medium', 'AP large',
                     'AR@1', 'AR@10', 'AR@100', 'AR samll', 'AR medium', 'AR large']
        elif iouType == 'keypoints':
            raise NotImplementedError
            metrics=[]
        
        single_result = {}
        single_result['image_id'] = im_id
        for metric, value in zip(metrics, self.stats):
            single_result[metric] = value
    ...
    def get_names(self):
        catIds = sorted(self.cocoGt.getCatIds())
        nm = [self.cocoGt.loadCats(catId)[0]['name'] for catId in catIds]
        return nm
