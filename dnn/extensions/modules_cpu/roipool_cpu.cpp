#include "roipool_cpu.hpp"

void RoiPoolForwardCpu( const float* data, const int batch_size, const int channels, const int height, const int width,
...
{
  int data_count = batch_size * channels * height * width;
  int crop_count = nrois * channels * pooled_height * pooled_width;
  ...
  //
  // data tensor shape [ batch_size, channels, height, width ]
  // crop tensor shape [ nrois, channels, pooled_height, pooled_width ]
  // argmax tensor shape [ nrois, channels, pooled_height, pooled_width ]

  for( int index=0 ; index<crop_count ; index++ )
  {
